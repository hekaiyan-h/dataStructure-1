...
Status GetElem(List L,int index,ElemType &e);//获取线性表index位置的元素 
Status LocateElem(List L,ElemType e);//判断e是否在L中， 不在返回false     
Status Append(List &L,ElemType e);//添加e到线性表末尾               
//Status SetAppend(List &L,ElemType e);//添加元素到集合中默认排序位置，默认按照字典顺序位置排序 
Status InsertElem(List &L,int index,ElemType e);//插入e到index位置      
Status removeElem(List &L,ElemType e);//删除线性表中e元素               
Status removeIndex(List &L,int index);//删除线性表中index号元素         
Status removalReapted(List &L);//去除线性表中重复元素                   
Status sort(List &L,bool Bool=true);//给线性表排序，默认升序  

Status printList(List L);
Status CreateSet(List &L,ElemType *s,int length);//创建集合                            
Status CreateSet(List &L);
Status DestroySet(List &L);//销毁集合                                   
Status Union(List L1,List L2,List &L3);//并集                           
Status Intersection(List L1,List L2,List &L3);//交集                    
Status Complement(List L1,List L2,List &L3);//补集                      
Status Difference(List L1,List L2,List &L3);//差集                      
...
Lab:	List set1;
	    ...
	    cout<<"\t\t\t"<<"1 交集:A∩B"<<"   "<<"2 并集:AUB"<<"   "<<"3 补集:!A（AUB为全集）"<<"  "<<"4 补集:!B（AUB为全集）"<<endl;
	    cout<<endl;
	    cout<<"\t\t\t"<<"5 差集:A-B"<<"       "<<"6 差集:B-A"<<"   "<<"7 重新输入集合"<<"    "<<"8 退出"<<endl;
	    cout<<"\t"<<"—————————————————————————————————————————————————"<<endl;
	    //cout<<"\t\t\t请选择操作:";
